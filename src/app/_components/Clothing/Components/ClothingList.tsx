...
import ComputersCard from '../../Computers/Components/ComoutersCard';
import GetProducts from '@/service/GetProducts';

const ClothingList = async() => {
  const products = await GetProducts();
  const chunkSize = 2; // Define the number of items per row

  const chunkedProducts = products?.results.slice(8, 20).reduce((resultArray, item, index) => {
